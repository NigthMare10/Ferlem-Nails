...
- Migraciones reales: existen las migraciones aditivas de `appointments`, `appointment_items`, segmentos por item, `appointment_events`, razón no-show, `appointment_deposits`, `appointment_deposit_refunds`, vínculo único `sales.appointment_id`, propósito de devolución, snapshots de checkout en `sale_items` y `sale_payments`. La orquestación final aplicó seis migraciones de 4C/4D en MySQL como batch 8 y la restauración de unicidad de líneas standalone como batch 9, sin `migrate:fresh`; 4F no creó migraciones.
...
- Pruebas automáticas históricas de 4F: 13 pruebas/230 aserciones; quedan sustituidas para el estado global por los resultados de Fase 5A documentados abajo.
- Verificación MySQL: `optimize:clear`, `migrate`, `db:seed`, `route:list` y `migrate:status` terminaron correctamente. Seis migraciones nuevas están en batch 8 y la corrección de unicidad standalone está en batch 9. Se conservaron 10 ventas, 15 líneas y 6 citas; las ventas históricas recibieron exactamente 10 pagos y no se crearon adelantos demo.
- Riesgos y validación manual pendiente: revisar planes SQL, volumen y concurrencia multiproceso MySQL; ejecutar E2E y comparación visual; validar filtros/scopes/compartidas/comprobantes/adelantos en 1440x900, 1024x768, 768x1024 y 390x844. SQLite sigue deshabilitado globalmente. `public/hot` está ausente y `public/build` queda como artefacto obligatorio de despliegue.

## Fase 5A - Estabilizacion final, notificaciones y produccion (2026-07-24)
...
- Produccion: Composer fija plataforma PHP 8.3 y dependencias compatibles; Symfony quedo en 7.4. `composer validate --strict`, `composer why-not php 8.3.0`, `composer check-platform-reqs` e instalacion `--no-dev` pasaron. TypeScript quedo en 5.9 con `tsconfig.json` y typecheck real. `.env.production.example`, `docs/HOSTINGER_DEPLOYMENT.md`, `scripts/deploy-hostinger.sh` y `scripts/smoke-test-production.sh` preparan Hostinger sin secretos.
- Verificacion: `php artisan test` directo ejecuto 254 pruebas, con 20 correctas/191 aserciones y 234 errores `could not find driver`; no se declara aprobado. Cargando SQLite solo en el proceso, 254/254 pruebas y 2,239 aserciones pasaron. La migracion y el seeder MySQL conservaron 13 ventas, 9 citas, 2 adelantos, 3 servicios y 4 usuarios. Pint completo, typecheck, caches Laravel, sintaxis shell, build y `git diff --check` pasaron.
- Pendiente manual: validar selector, checkout normal/cita efectivo/tarjeta/adelanto, comprobante, Ganancias simplificada, campana/listado/RBAC, responsive, impresion y despliegue real. Solo el usuario puede aprobar 4B-4F y 5A.
